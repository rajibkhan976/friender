...
import { CalendarIcon, DeleteIcon, EditIcon, OpenInNewTab, ThreeDotIcon } from "../../../assets/icons/Icons";
import { updateCampaignContext, updateCampaignsArray } from "../../../actions/MessageAction";
import useComponentVisible from "../../../helpers/useComponentVisible";
import Alertbox from "../../common/Toast";

export const CampaignNameCellRenderer = memo((params) => {
    const storeEdit = () => {
        localStorage.setItem('fr_editCampaign_view', JSON.stringify({
            mode: 'view'
        }))
    }
...
export const CampaignStatusCellRenderer = memo((params) => {
    ...
    const doSomething = (e) => {
        if (params?.data?.friends_pending === 0 || new Date(params?.data?.campaign_end_time) < new Date()) {
            Alertbox(
...
export const CampaignFriendsCountCellRenderer = memo((params) => {
    return (
        <div className="campaign-count-cell">
            {params?.value}
        </div>
    )
})
...
export const CampaignEndTimeCellRenderer = memo((params) => {
    ...
    const convertedTimeSplit = utils.convertUTCtoLocal(params?.value?.replace(" ", "T") + ".000Z", true).split(",");

    return (
        <div className={`campaign-endTime-cell ${new Date() > new Date(params?.value) ? 'end-time-exceeded' : ''}`}>
            {params?.value ?
                <>
                    <CalendarIcon />&nbsp;{convertedTimeSplit[0]}, {convertedTimeSplit[1]?.toLowerCase()}
...
export const CampaignContextMenuCellRenderer = (params) => {
    ...
    const togggleContext = () => {
        ...
        dispatch(
            updateCampaignContext(params?.data?._id)
        )
    }
    ...
    return (
        ...
        >
            <div
                className={isComponentVisible ? "message-context-menu active" : 'message-context-menu'}
            >
                <button
                ...
                {isComponentVisible &&
                    <div className="context-menu">
                        ...
                        ><span className="context-icon"><EditIcon /></span>Edit</button>
                        <button className="btn btn-delete"
                            onClick={deleteCampaign}
                        ><span className="context-icon"><DeleteIcon /></span>Delete</button>
                    </div>
                }
            </div>
        </div>
