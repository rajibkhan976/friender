...
import { AddTransparentIcon, ChevronLeftArrowIcon } from "../../../assets/icons/Icons";
...
const CampaignsHeader = ({
    ...
    const switchPauseCampaign = (e) => {
        if (editCampaign?.friends_pending === 0 || new Date(editCampaign?.campaign_end_time) < new Date()) {
            Alertbox(
    ...
    useEffect(() => {
        ...
        if (params?.campaignId) {
            ...
            if (localStorage?.getItem('fr_editCampaign_view') && localStorage?.getItem('fr_editCampaign_view') != 'undefined') {
                if (JSON.parse(localStorage.getItem('fr_editCampaign_view')).mode !== editOptions?.find(el => el.checked).label) {
                    let updatedViewObject = editOptions?.find(el => el.label === JSON.parse(localStorage.getItem('fr_editCampaign_view')).mode)
                    ...
                    <>
                        {
                            (editCampaign || editCampaign !== null || params?.campaignId) ?
                                <>
                                    ...
                                    <h3>
                                        {editCampaign ? editCampaign?.campaign_name: 'Loading your campaign'}
                                    </h3>
