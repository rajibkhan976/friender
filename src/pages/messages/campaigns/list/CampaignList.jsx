  ...
  const checkAll = e => {
    ...
    const fetchListData = async () => {
        ...
        try {
            ...
            if (sorting && Array.isArray(sorting) && sorting?.length) {
                Object.assign(urlSearchParamsObject, { sort_by: sorting[0]?.id, sort_order: sorting[0]?.desc ? "desc" : "asc" });
            }

            if (pagination) {
                Object.assign(urlSearchParamsObject, { page_number: pagination?.pageIndex + 1, page_size: pagination?.pageSize });
            }

            console.log(urlSearchParamsObject);

            const response = await fetchAllCampaign(Object.keys(urlSearchParamsObject)?.length ? urlSearchParamsObject : null);
            console.log("Campaign data", response);
            dispatch(updateCampaignsArray(response?.data));
            setData(response?.data);
            setRowCount(response?.total_campaings);
            dispatch(updateCurrlistCount(response?.total_campaings));
        } catch (error) {
            setIsError(true);
            console.error(error);
            return;
    ...
    useEffect(() => {
        if (props.radioOption && 
			...
			props.radioOption?.length && 
			props.radioOption.some((item) => item?.label === "listing" && item?.checked)
		) {
            fetchListData();
        }
    }, [campaignFilter, campaignDuration, pagination.pageIndex, pagination.pageSize, props.radioOption, sorting]);
