...
const MySetting = () => {
	...
	const [dontSendFrindReqThyRejct, setDontSendFrindReqThyRejct] =
		useState(false);
	const [dontSendFrindReqFbRejct, setDontSendFrindReqFbRejct] = useState(false);
	...
	const [deletePendingFrndModalOpen, setDeletePendingFrndModalOpen] =
		useState(false);
	const [deletePendingFrndStartFinding, setDeletePendingFrndStartFinding] =
		useState(false);
	...
	const [selectMsgTempAcceptsFrndReq, setSelectMsgTempAcceptsFrndReq] =
		useState(false);
	const [sndMsgAcptsFrndReqGroupSelect, setSndMsgAcptsFrndReqGroupSelect] =
		useState(null);
	const [quickMsgAcptsFrndReq, setQuickMsgAcptsFrndReq] = useState(null);
	const [sndMsgAcptsQuickMsgModalOpen, setSndMsgAcptsQuickMsgOpen] =
		useState(false);
	...
	const [selectMsgTempRejectFrndReq, setSelectMsgTempRejectFrndReq] =
		useState(false);
	const [sndMsgRejtFrndReqGroupSelect, setSndMsgRejtFrndReqGroupSelect] =
		useState(null);
	const [quickMsgRejtFrndReq, setQuickMsgRejtFrndReq] = useState(null);
	const [sndMsgRejtQuickMsgModalOpen, setSndMsgRejtQuickMsgOpen] =
		useState(false);
	const [usingSelectOptions2, setUsingSelectOptions2] = useState(false);

	// Send message when someone sends me a friend request..
	const [sndMsgSomeoneSndFrndReqToggle, sertSndMsgSomeoneSndFrndReqToggle] =
		useState(false);
	const [selectMsgSomeoneSndFrndReq, setSelectMsgSomeoneSndFrndReq] =
		useState(false);
	const [
		sndMsgSomeoneSndFrndReqGroupSelect,
		setSndMsgSomeoneSndFrndReqGroupSelect,
	] = useState(null);
	const [quickMsgSomeoneSndFrndReq, setQuickMsgSomeoneSndFrndReq] =
		useState(null);
	const [
		sndMsgSomeoneSndFrndReqQuickMsgModalOpen,
		setSndMsgSomeoneSndFrndReqQuickMsgModalOpen,
	] = useState(false);
	const [usingSelectOptions3, setUsingSelectOptions3] = useState(false);

	// Send message when I reject an incoming friend request..
	const [sndMsgRejtIncomingFrndReqToggle, setSndMsgRejtIncomingFrndReqToggle] =
		useState(false);
	const [selectMsgRejtIncomingFrndReq, setSelectMsgRejtIncomingFrndReq] =
		useState(false);
	const [
		sndMsgRejtIncomingFrndReqGroupSelect,
		setSndMsgRejtIncomingFrndReqGroupSelect,
	] = useState(null);
	const [
		quickMsgRejtIncomingFrndReqFrndReq,
		setQuickMsgRejtIncomingFrndReqFrndReq,
	] = useState(null);
	const [
		sndMsgRejtIncomingFrndReqQuickMsgModalOpen,
		setSndMsgRejtIncomingFrndReqQuickMsgModalOpen,
	] = useState(false);
	...
	const [selectMsgAcptsIncomingFrndReq, setSelectMsgAcptsIncomingFrndReq] =
		useState(false);
	const [
		sndMsgAcptsIncomingFrndReqGroupSelect,
		setSndMsgAcptsIncomingFrndReqGroupSelect,
	] = useState(null);
	const [
		quickMsgAcptsIncomingFrndReqFrndReq,
		setQuickMsgAcptsIncomingFrndReqFrndReq,
	] = useState(null);
	const [
		sndMsgAcptsIncomingFrndReqQuickMsgModalOpen,
	...
	const [sndMsgExptFrndRquOpenSelect, setsndMsgExptFrndRquOpenSelect] =
		useState(periodObj[0].value);
	...
	const [sndMsgExptFrndRquOpenInput, setsndMsgExptFrndRquOpenInput] =
		useState(1);
	...
	const [sndMsgExptFrndRquOpenMsgSelect, setsndMsgExptFrndRquOpenMsgSelect] =
		useState(msgTmpltObj[0]);
	...
	useEffect(() => {
		...
		if (
			localStorage.getItem("fr_delete_id") ===
		...
		dispatch(getMySettings({ fbUserId: `${current_fb_id}` }))
			...
			.then((res) => {
				...
				if (
					!localStorage.getItem("fr_inactive_after") ||
					localStorage.getItem("fr_inactive_after") == "undefined"
				) {
					localStorage.setItem(
						"fr_inactive_after",
						res?.data[0]?.friends_willbe_inactive_after
					);
				}
			})
			.catch((err) => {
	...
	useEffect(() => {
		...
		dispatch(getMySettings({ fbUserId: `${current_fb_id}` }))
			...
			.then((res) => {
				...
				const responsedAPIReFrndingKeywords =
					responseData?.re_friending_settings[0]?.keywords;
				const fbUserIdFromAPI = current_fb_id;
				const fbUserIdFromBrowser = JSON.parse(
					localStorage.getItem("fr_refriending_data")
				)?.current_fb_id;
				...
				if (fbUserIdFromAPI !== fbUserIdFromBrowser) {
					...
					const existingStorageData = JSON.parse(
						localStorage.getItem("fr_refriending_data") || "{}"
					);
					existingStorageData.keywords = responsedAPIReFrndingKeywords;
					localStorage.setItem(
						"fr_refriending_data",
						JSON.stringify(existingStorageData)
					);
				}
			})
			.catch((err) => {
	...
	const saveMySetting = (withSaveButton = false) => {
		const payload = {
			...
			dont_send_friend_requests_prople_i_sent_friend_requests_they_rejected:
				dontSendFrindReqThyRejct,
			avoid_sending_friend_request_to_restricted_people:
				dontSendFrindReqFbRejct,
			...
			//.. New Payload..
			send_message_when_someone_sends_me_friend_request:
				sndMsgSomeoneSndFrndReqToggle,
			...
			send_message_when_reject_incoming_friend_request:
				sndMsgRejtIncomingFrndReqToggle,
			...
			send_message_when_accept_incoming_friend_request:
				sndMsgAcptsIncomingFrndReqToggle,
		...
		if (reFrndng) {
			if (
				...
				payload.re_friending_settings = {
					...
					keywords:
						localStorage.getItem("fr_refriending_data") &&
						JSON.parse(localStorage.getItem("fr_refriending_data") || "")
							?.keywords,
				};
	...
	const syncSettings = (data) => {
		...
		setDontSendFrindReqFbRejct(
			data.avoid_sending_friend_request_to_restricted_people
		);
		...
		if (data?.automatic_cancel_friend_requests_settings.length > 0) {
		...
		if (data.send_message_when_someone_accept_new_friend_request_settings) {
			...
			if (
				...
				dispatch(getGroupById(message_group_id))
					.unwrap()
					.then((res) => {
						const data = res?.data;
		...
		if (data.send_message_when_reject_friend_request_settings) {
			const { message_group_id, quick_message } =
				data?.send_message_when_reject_friend_request_settings[0];
		...
		sertSndMsgSomeoneSndFrndReqToggle(
			data?.send_message_when_someone_sends_me_friend_request
		);

		if (data.send_message_when_someone_sends_me_friend_request_settings) {
			const { message_group_id, quick_message } =
				data?.send_message_when_someone_sends_me_friend_request_settings[0];
		...
		setSndMsgRejtIncomingFrndReqToggle(
			data?.send_message_when_reject_incoming_friend_request
		);

		if (data.send_message_when_reject_incoming_friend_request_settings) {
			const { message_group_id, quick_message } =
				data?.send_message_when_reject_incoming_friend_request_settings[0];
		...
		setSndMsgAcptsIncomingFrndReqToggle(
			data?.send_message_when_accept_incoming_friend_request
		);

		if (data.send_message_when_accept_incoming_friend_request_settings) {
			const { message_group_id, quick_message } =
				data?.send_message_when_accept_incoming_friend_request_settings[0];
	...
	const deletePendingRequestHandle = async () => {
		...
		checkDeletePFRProgress();
		await extensionAccesories.sendMessageToExt({
			action: "deletePendingFR",
							...
							<div className={"input-num"}>
								...
								<div className='input-arrows'>
									<button
										...
										onClick={() =>
											setValOfFrndWillInactiveAfterDays("INCREMENT")
										}
									>
									...
									<button
										className='btn inline-btn btn-transparent'
										onClick={() =>
											setValOfFrndWillInactiveAfterDays("DECREMENT")
										}
									>
              ...
              /> */}
						...
						{reFrndng && (
							<div className='setting-child'>
								...
								>
									<div
										className={!reFrndng ? "input-num disabled" : "input-num"}
									>
										<input
											type='number'
											className='setting-input'
								...
								<span className='smallTxt'>
									{" "}
									day(s), and immediately send a new friend request. Choose how
									often to retry friending up to
								</span>{" "}
									...
									<div className='saveBlock'>
										...
										<span className='smallTxt'>
											...
											<ToolTipPro
												isInteract={false}
												textContent={
													"Enabling this feature will resend friend request(s) to those targeted friends who have matched the designated keyword(s)"
												}
												type={"info"}
											/>
										</span>{" "}
										{reFriendSaveActive && reFriendOpenKeywords && (
											<>
												...
												{/* add class "activated" for blue color */}
												<span className='doomedText'>
													* Enter a comma after each keyword
												</span>
											</>
										)}
										{!reFriendSaveActive &&
											reFrndngKeywords?.length > 0 &&
											reFriendOpenKeywords && (
												<>
													<button
														className={`saveBtn activated`}
						...
						<div
							className={`setting ${autoCnclFrndRque ? "setting-actived" : ""}`}
						>
							<Switch
							...
							<div className='setting-control'>
								...
								>
									{!deletePendingFrndStartFinding ? (
										<>
											<span>
												<OutlinedDeleteIcon color='white' />
											</span>
											Delete all
										</>
									) : (
						...
						{autoCnclFrndRque && (
							<div className='setting-child others'>
								...
								>
									<div
										className={
											!autoCnclFrndRque ? "input-num disabled" : "input-num"
										}
									>
										<input
											type='number'
										...
										<div className='input-arrows'>
											<button
												className='btn inline-btn btn-transparent'
												onClick={() =>
													setValOfDeletePendingFrndIncDic("INCREMENT")
												}
											>
												<ChevronUpArrowIcon size={15} />
											</button>
											...
											<button
												className='btn inline-btn btn-transparent'
												onClick={() =>
													setValOfDeletePendingFrndIncDic("DECREMENT")
												}
											>
												<ChevronDownArrowIcon size={15} />
											</button>
								...
								day(s)
								{deletePendingFrndError && (
									<span className='error-text'>
										Provided value must be within the range of 1 to 31
									</span>
								)}
							</div>
						)}
								...
								<div className='custom-select-drop-area'>
									...
									<TurnOnSettingsWarn enabledFeature={sndMsgAcptsFrndReqToggle}>
										<DropSelectMessage
											type='ACCEPT_REQ'
											openSelectOption={
												selectMsgTempAcceptsFrndReq && sndMsgAcptsFrndReqToggle
											}
											...
											quickMessage={
												quickMsgAcptsFrndReq && quickMsgAcptsFrndReq
											}
								...
								<div className='custom-select-drop-area'>
									...
									<TurnOnSettingsWarn enabledFeature={sndMsgRejtFrndReqToggle}>
										<DropSelectMessage
											type='REJECT_REQ'
											openSelectOption={
												selectMsgTempRejectFrndReq && sndMsgRejtFrndReqToggle
											}
											handleIsOpenSelectOption={
												sndMsgRejtFrndReqToggle && setSelectMsgTempRejectFrndReq
											}
											groupList={groupsToSelect}
						...
						{sndMsgSomeoneSndFrndReqToggle && (
							<div className='setting-child others'>
								<div className='custom-select-drop-area'>
									Select the message template you want to send &nbsp;
									<TurnOnSettingsWarn
										enabledFeature={sndMsgSomeoneSndFrndReqToggle}
									>
										<DropSelectMessage
											...
											quickMessage={
												quickMsgSomeoneSndFrndReq && quickMsgSomeoneSndFrndReq
											}
						...
						{sndMsgRejtIncomingFrndReqToggle && (
							<div className='setting-child others'>
								<div className='custom-select-drop-area'>
									Select the message template you want to send &nbsp;
									<TurnOnSettingsWarn
										enabledFeature={sndMsgRejtIncomingFrndReqToggle}
									>
										<DropSelectMessage
						...
						{sndMsgAcptsIncomingFrndReqToggle && (
							<div className='setting-child others'>
								<div className='custom-select-drop-area'>
									Select the message template you want to send &nbsp;
									<TurnOnSettingsWarn
										enabledFeature={sndMsgAcptsIncomingFrndReqToggle}
									>
										<DropSelectMessage
