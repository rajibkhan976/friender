...
import { ListingIcon, CampaignsCalendarIcon, SettingIconLarge, UserIcon } from "../../../assets/icons/Icons";
...
const Campaigns = () => {
    ...
    const changeCampaignsView = (checkThis) => {
        try {
            ...
            radioOptionPlaceholder = radioOptionPlaceholder?.map(el => el?.label !== checkThis?.label ? { ...el, checked: false } : { ...checkThis })
    ...
    const changeSpanView = (el) => {
        setSpanOption(prevStatus => prevStatus?.map(e => ({ ...e, selected: e.value != el?.target?.value ? false : true })))
    }

    // TOGGLE ACTIVE / INACTIVE CAMPAIGNS FOR CAMPAIGNS LIST
    const changeStatusView = (el) => {
        setStatusOption(prevStatus => prevStatus?.map(e => ({ ...e, selected: e.value != el?.target?.value ? false : true })))
        filterCampaigns()
    }
    ...
    const changeEditView = (el) => {
        ...
        editViewPlaceholder = editViewPlaceholder.map(item => item.label === el.label ? { ...el } : { ...item, checked: false });
    ...
    useEffect(() => {
        ...
        if (isEditingCampaign) {
            ...
            campaignsCreatedPlaceholder = campaignsCreatedPlaceholder?.map(el => el?._id !== isEditingCampaign?._id ? { ...el } : { ...isEditingCampaign })
