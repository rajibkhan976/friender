...
const Sidebar = (props) => {
 ...
 const [sidebarOpenFriends, setSidebarOpenFriends] = useState(false);
  ...
  useEffect(()=>{
    ...
    // console.log("The Aunticate state is", authenticated);
    // console.log("The passwordTokrn is",resetpassword_token);
    // console.log("The OnboardingToken is",onboarding_token);
...
const closePopupFn = (e) => {
  ...
  return (
    <aside className={sidebarToogle ? "sidebar closed d-flex d-flex-column f-justify-start" : "sidebar d-flex d-flex-column f-justify-start"}>
      ...
      <>
      {authenticated && 
      <span className={sidebarToogle ? "menu-toogle closed" : "menu-toogle"} onClick={setSidebarToogleFn}>
        
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g opacity="0.7">
            <path d="M4.5 9L3.79289 9.70711L3.08579 9L3.79289 8.29289L4.5 9ZM8.29289 14.2071L3.79289 9.70711L5.20711 8.29289L9.70711 12.7929L8.29289 14.2071ZM3.79289 8.29289L8.29289 3.79289L9.70711 5.20711L5.20711 9.70711L3.79289 8.29289Z" fill="white"/>
            <path d="M9 9L8.29289 9.70711L7.58579 9L8.29289 8.29289L9 9ZM12.7929 14.2071L8.29289 9.70711L9.70711 8.29289L14.2071 12.7929L12.7929 14.2071ZM8.29289 8.29289L12.7929 3.79289L14.2071 5.20711L9.70711 9.70711L8.29289 8.29289Z" fill="white"/>
            </g>
          </svg>
       </span>
      }
      ...
      <div className="sidebar-opened-wraper d-flex d-flex-column f-justify-start">
        <div className="sidebar-top-wraper d-flex">
          ...
          <figure className={authenticated ? "sidebar-logo logo-closed": "sidebar-logo logo-closed no-click"}>
            <NavLink to="/">
        ...
        <li className="nav-menu" onClick={setSidebarHomeOpenFn}>
          ...
          <li className={sidebarOpenFriends ? "nav-menu has-child activated" : "nav-menu has-child"} onClick={setSidebarOpenFn}>
            <NavLink to="/friends/friend-list" onClick={setSubMenuFriendsFn}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M18.2283 18.6183C18.6422 18.5265 18.8901 18.0956 18.7013 17.716C18.2172 16.7422 17.4295 15.8865 16.4117 15.2399C15.146 14.4358 13.5953 14 12 14C10.4047 14 8.85398 14.4358 7.58835 15.2399C6.57054 15.8865 5.78285 16.7422 5.29865 17.716C5.10992 18.0956 5.35784 18.5265 5.77169 18.6183C9.87403 19.5284 14.126 19.5284 18.2283 18.6183Z" fill="#BDBDBD"/>
                <ellipse cx="12" cy="7.5" rx="4.375" ry="4.5" fill="#BDBDBD"/>
              </svg>
              <span className="nav-menu-name">Friends  <span className={subMenuFriends ? "sub-menu-toogle": "sub-menu-toogle sub-closed"}><img src={whiteArrow} alt=""/></span></span>
            </NavLink>
            {subMenuFriends &&
      ...
      {authenticated && 
      <nav className="nav-bar bottom-nav-only m-top-a closed-only" onClick={setSidebarHomeOpenFn}>
      ...
      <ul className="bottom-nav-bar m-top-a">
        {sidebarToogle  && 
            <>
              <li className={authenticated ? "nav-menu closed-only" : "nav-menu closed-only no-click"}>
                  <NavLink to="/">
                    <svg
                      width="24"
                      height="24"
                      ...
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <circle cx="12" cy="12" r="9" stroke="#BDBDBD" strokeWidth="2" />
                      <circle cx="12" cy="17" r="0.5" fill="#BDBDBD" stroke="#BDBDBD" />
                      <path
...
<li className={showProfile ? "nav-menu user-profile-image profile-opened" : "nav-menu user-profile-image"}>
            <span className="profile-photo" onClick={setShowProfileFn} style={{
                          backgroundImage: `url(${profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture ? profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture : ProfilePhoto})`
                        }}>
              {/* <img src={profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture} alt="" /> */}
            </span>          
</li>
              <li className="nav-menu closed-only">
                  <button className="btn-transparent logout-btn" onClick={logoOut}>
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
        ...
        {!sidebarToogle  && 
          ...
          <li className={showProfile ? "nav-menu user-profile-image profile-opened" : "nav-menu user-profile-image"}>
            ...
            <button className="btn-transparent menu-detail">
              <svg width="46" height="40" viewBox="0 0 46 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="0.5" y="0.5" width="45" height="39" rx="4.5" fill="#605BFF" fillOpacity="0.1" stroke="#605BFF"/>
                <circle cx="21.5" cy="17" r="3.75" fill="#605BFF"/>
                <path d="M28.25 18.5L28.25 23" stroke="#605BFF" strokeWidth="2" strokeLinecap="round"/>
                <path d="M30.5 20.75L26 20.75" stroke="#605BFF" strokeWidth="2" strokeLinecap="round"/>
                <path d="M26.8566 26.2869C27.2028 26.2085 27.4105 25.8486 27.257 25.5285C26.8432 24.6652 26.163 23.9066 25.2814 23.3349C24.1966 22.6313 22.8674 22.25 21.5 22.25C20.1326 22.25 18.8034 22.6313 17.7186 23.3349C16.837 23.9066 16.1568 24.6652 15.743 25.5284C15.5895 25.8486 15.7972 26.2085 16.1434 26.2869C19.6698 27.0855 23.3302 27.0855 26.8566 26.2869Z" fill="#605BFF"/>
                </svg>
              </button>
            </li>
            <li className="nav-menu opened-only">
            ...
            {showProfile && 
            <>
                <div className={authenticated ? "profile-popup" : "profile-popup hidden"}>
                  <div className="profile-listings-sec-wraper">
                    <div className="main-profile-selected">
                      {/* <img src={profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture} alt="" /> */}
                      <img src={profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture ? profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture : ProfilePhoto} alt="" />
                    </div>
                    <div className="profile-option-listings">
                      {profiles?.filter(el => el.fb_user_id != local_fb_id).length > 0 ? <ul>
                        {
                          profiles?.filter(el => el.fb_user_id != local_fb_id).map((el, key) => {
                            return (
                              <li key={key} data-tooltip={el.name}>
                                <span className="sub-profiles" onClick={() => clickInImage(el.fb_user_id)}>
                                  <span>
                                    <img src={el?.fb_profile_picture} alt="" />
                                  </span>
                                </span>
                              </li>
                            )
                    ...
                    <button className="add-new-profile text-center btn-transparent">
                        <svg width="38" height="38" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M22 16H20V20H16V22H20V26H22V22H26V20H22V16ZM21 11C15.48 11 11 15.48 11 21C11 26.52 15.48 31 21 31C26.52 31 31 26.52 31 21C31 15.48 26.52 11 21 11ZM21 29C16.59 29 13 25.41 13 21C13 16.59 16.59 13 21 13C25.41 13 29 16.59 29 21C29 25.41 25.41 29 21 29Z" fill="#767485"/>
                          <rect x="1" y="1" width="40" height="40" rx="20" stroke="#767485" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 8"/>
                        </svg>
                      </button>
                    </div>
                  <div className="current-profile-info-wraper">
                      <button className="btn-transparent close-popup"  onClick={setShowProfileFn}>
                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M13.5 4.5L4.5 13.5" stroke="#9A9AA9" strokeLinecap="round" strokeLinejoin="round"/>
                          <path d="M4.5 4.5L13.5 13.5" stroke="#9A9AA9" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      </button>
                    <div className="profile-inner-wraper">
                    <div className="profile-popup-info">
                      <div className="img-section">
                        <img src={profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture ? profiles?.filter((el) => el.fb_user_id == local_fb_id)[0]?.fb_profile_picture : ProfilePhoto} className="profile-main" alt="" />
                        <span className="profile-status active"></span>
                      </div>
                      <div className="profiles-informations">
                        {console.log('profiles', profiles)}
                        <p>{profiles?.filter(el => el.fb_user_id == local_fb_id)[0]?.name ? profiles?.filter(el => el.fb_user_id == local_fb_id)[0]?.name : 'Anonymous'}</p>
                        <span>{userEmail}</span>
                      </div>
                    </div>
                    ...
                    <div className="popup-menu" onClick={closePopupFn}>
                      <ul>
                        ...
                        <li><NavLink to="/settings/my-settings">My Settings</NavLink></li>
                        <li><NavLink to="/">Rewards</NavLink></li>
                        <li><NavLink to="/">Trash</NavLink></li>
                        <li><NavLink onClick={logoOut}>Logout</NavLink></li>
                      </ul>
                      </div>
                      <div className="popup-menu lower-menu">
                      <button className="btn-primary">Upgrade Now</button>
                      <ul>
                        <li onClick={closePopupFn}><NavLink to="/">Help</NavLink></li>
                        <li className="mode-changer">Dark Mode 
                          <Themewitch
                            extraClass="mode-change-sidebar"
                          />
                        </li>
                      </ul>
                    </div>
