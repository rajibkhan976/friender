...
const OnboardingPage = () => {
  ...
  useEffect(() => {
    // console.log( "one",selectedValueOne);
    // console.log( "two",selectedValueTwo);
    // console.log( "three",selectedValueThree);
  }, [selectedValueOne, selectedValueTwo, selectedValueThree]);
  ...
  const handelerSubmit = (e) => {
    ...
    dispatch(
      ...
      .then((response) => {
        ...
        console.log("fr_onboarding after is :", token_onboarding);
  ...
  return (
    ...
    <div className="page-wrapers">
      <div className={module["auth-heading-info"]} >
        <h3 className="text-center onboarding-heading">Tell us about yourself</h3>
        <p className="text-center onboarding-para">We’ll cusmotize your friender exprience besed on your choice.</p>
      </div>
      <form onSubmit={handelerSubmit} >
        <div className="selectbox-wraper">
          <p>What kind of work did you do?</p>
          <DropSelector selects={select_one} value={selectedValueOne} handleChange={(e)=>{ setSelectedValueOne(e.target.value)}} width={"100%"}  />
        </div>
        <div className="selectbox-wraper">
          <p>What is your role?</p>
          <DropSelector selects={select_two} handleChange={(e)=>{ setSelectedValueTwo(e.target.value)}} width={"100%"}  />
        </div>
        <div className="selectbox-wraper">
          <p>What are you planing to do in Friender?</p>
          <DropSelector selects={select_three} handleChange={(e)=>{ setSelectedValueThree(e.target.value)}} width={"100%"} />
        </div>
        {success && (
        ...
        {selectedValueOne === "null" && selectedValueTwo === "null" && selectedValueThree === "null" ? (
       
            <Button extraClass="btn-primary" btnText="Next" loaderValue={loader}  disable={true} />
          ) : (
             <Button extraClass="btn-primary" btnText="Next" loaderValue={loader} />
        )}
        </div>
      </form>
    </div>
